...
class Configuration {
	...
	public static function dataDir() {
		if( ! self::$dataDir ) {
			throw new \Codeception\Exception\Configuration( 'Path for data not specified. Please, set data path in global config' );
		}
		return self::$dir . self::$dataDir . DIRECTORY_SEPARATOR;
	}

	public static function helpersDir() {
		return self::$dir . self::$helpersDir . DIRECTORY_SEPARATOR;
	}

	public static function logDir() {
		if( ! self::$logDir ) {
			throw new \Codeception\Exception\Configuration( 'Path for logs not specified. Please, set log path in global config' );
		}
		$dir = realpath( self::$dir . self::$logDir ) . DIRECTORY_SEPARATOR;
		if( ! is_writable( $dir ) ) {
			@mkdir( $dir );
			@chmod( $dir, 777 );
		}
	...
	public static function suiteSettings( $suite, $config ) {
		...
		$suiteConf     = file_exists( self::$dir . $path . DIRECTORY_SEPARATOR . $suite . '.suite.yml' ) ? Yaml::parse( self::$dir . $path . DIRECTORY_SEPARATOR . $suite . '.suite.yml' ) : array();
		$suiteDistconf = file_exists( self::$dir . $path . DIRECTORY_SEPARATOR . $suite . '.suite.dist.yml' ) ? Yaml::parse( self::$dir . $path . DIRECTORY_SEPARATOR . $suite . '.suite.dist.yml' ) : array();
		...
		$settings[ 'path' ] = self::$dir . $path . DIRECTORY_SEPARATOR . $suite . DIRECTORY_SEPARATOR;
	...
	public static function modules( $settings ) {
		...
		if( ! isset( $settings[ 'modules' ] ) ) {
			throw new \Codeception\Exception\Configuration( 'No modules configured!' );
		}
